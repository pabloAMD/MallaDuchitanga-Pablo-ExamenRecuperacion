...
import ec.edu.ups.ejb.DepositoFacade;
...
public class DepositoBean implements Serializable {
	...
	@EJB
	private DepositoFacade ejbDeposito;
	...
	@PostConstruct
	public void init() {
		list= ejbDeposito.findAll();
		cuentaBancariaList= ejbCuentaBancaria.findAll();
	}

	public DepositoFacade getEjbDeposito() {
		return ejbDeposito;
	}

	public void setEjbDeposito(DepositoFacade ejbDeposito) {
		this.ejbDeposito = ejbDeposito;
	}

	public Deposito[] getList() {
		return list.toArray(new Deposito[0]);
	}
	...
	public String add() {
		...
		ejbDeposito.create(new Deposito(this.fecha, this.hora, this.descripcion, this.montoTotal, c));	
		list = ejbDeposito.findAll();
		return null;
	}
